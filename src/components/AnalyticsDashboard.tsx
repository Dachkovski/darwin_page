...
export default function AnalyticsDashboard({ data }: { data: ChartDataPoint[] }) {
  ...
  const latestGen = chartData.length > 0 ? chartData[chartData.length - 1] : null;
  const previousGen = chartData.length > 1 ? chartData[chartData.length - 2] : null;
              ...
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  ...
                  formatter={(value: number) => {
                    if (['score', 'visitors'].includes(activeMetric)) return [value.toFixed(2), ''];
